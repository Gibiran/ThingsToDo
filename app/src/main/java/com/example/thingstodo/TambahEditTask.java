...
public class TambahEditTask extends AppCompatActivity {
    ...
    public void saveTask(View v) {
        ...
        if (edit_text.getText().toString().trim().length() > 0) {
            ...
            if (isModify) {
                ...
                Toast.makeText(getApplicationContext(), "Tugas diupdate", Toast.LENGTH_SHORT).show();
            } else {
                mydb.insertTask(edit_text.getText().toString(), new SimpleDateFormat("yyyy-MM-dd").format(calendar.getTime()));
                Toast.makeText(getApplicationContext(), "Tugas ditambah", Toast.LENGTH_SHORT).show();
            }
            finish();

        } else {
            Toast.makeText(getApplicationContext(), "Anda tidak bisa menyimpan tugas kosong :/", Toast.LENGTH_SHORT).show();
        }
    ...
    public void deleteTask(View v) {
        ...
        Toast.makeText(getApplicationContext(), "Tugas dihapus", Toast.LENGTH_SHORT).show();
        finish();
    }


    public void chooseDate(View view) {
        final View dialogView = View.inflate(this, R.layout.pick_tanggal, null);
